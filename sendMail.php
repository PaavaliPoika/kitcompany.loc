...
if ( isset( $_POST['sendMail'] ) ) {
  $name  = substr( $_POST['user_name_modal'], 0, 64 );
  $phone  = substr( $_POST['user_phone_modal'], 0, 64 );
  $email   = substr( $_POST['user_email_modal'], 0, 64 );
  $subject = substr( $_POST['subject_modal'], 0, 64 );
  $message = substr( $_POST['message_modal'], 0, 250 );
  ...
  if ( !empty( $email ) and !preg_match( "#^[0-9a-z_\-\.]+@[0-9a-z\-\.]+\.[a-z]{2,6}$#i", $email ) )
    $error = $error.'<li>поле "E-mail" должно соответствовать формату user@example.com</li>';
  if ( !empty( $error ) ) {
    $_SESSION['sendMailForm']['error']   = '<p>При заполнении формы были допущены ошибки:</p><ul>'.$error.'</ul>';
    $_SESSION['sendMailForm']['user_name_modal']    = $name;
    $_SESSION['sendMailForm']['user_phone_modal']    = $phone;
    $_SESSION['sendMailForm']['user_email_modal']   = $email;
    $_SESSION['sendMailForm']['subject_modal'] = $subject;
    $_SESSION['sendMailForm']['message_modal'] = $message;
